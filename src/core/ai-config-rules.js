...
var CAPABILITY_ROUTING = [
  '- Establish `ui-impact` for each new Spec from exact Profile / `affected-units`; when unknown ask once, and for frontend or mixed tasks record `visual-context-intent` once (`not-required|direction|fidelity`).',
  '- Run `sdd visual discover` only to scan local Context candidates. A Figma URL is recorded like any other URL and does not access the network. The Agent does not automatically approve visual evidence. Do not fabricate a browser result or screenshot diff.',
  '- Strict visual evidence activates only when the current user explicitly runs `sdd visual init`; afterward follow the approved contract. The Agent must never create, approve, or replace a baseline and must not implicitly initialize or approve its Provider.',
  '- When engineering facts are insufficient, run read-only `sdd profile detect`; during Design / Acceptance / Plan, map AC `Verification:` values with read-only `sdd quality plan`. Profile `commandRefs` are facts and must not be executed automatically. These projections must not install dependencies or browsers, initialize or approve a Provider, or execute verification.'
];
