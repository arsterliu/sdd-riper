...
var codemapScan = require('../core/codemap-scan');
...
function run(projectDir, moduleName, opts) {
  ...
  if (fs.existsSync(outFile) && !(opts && opts.force)) { console.error('[ERROR] Already exists. Use --force.'); process.exit(2); }

  var content;
  if (opts && opts.scan) {
    var scanResult = codemapScan.scan(projectDir);
    content = codemapScan.renderCodemap(moduleName, projectDir, scanResult);
  } else {
    content = SKELETON
