...
function evaluateProcessResult(processResult, endEvent) {
  processResult = processResult || {};
  if (processResult.error && processResult.error.code === 'ETIMEDOUT') {
    return { status: 'blocked', gateDecision: 'BLOCKED', code: 'PROCESS_TIMEOUT' };
  }
  ...
  if (endEvent.status === 'interrupted' || endEvent.status === 'timedout') {
    return { status: 'interrupted', gateDecision: 'BLOCKED', code: 'REPORTER_INTERRUPTED' };
  }
...
module.exports = { evaluateProcessResult: evaluateProcessResult };
