...
out=$(bash "$SDD" build-context-bundle "$tmp" --sources "$src_dir") && exit_code=0 || exit_code=$?
...
bash "$SDD" init "$tmp" --mode standard >/dev/null
out=$(bash "$SDD" build-context-bundle "$tmp" --sources "$tmp/nonexistent" 2>&1) && exit_code=0 || exit_code=$?
...
bash "$SDD" init "$tmp" --mode standard >/dev/null
touch "$tmp/file.txt"
out=$(bash "$SDD" build-context-bundle "$tmp" --sources "$tmp/file.txt" 2>&1) && exit_code=0 || exit_code=$?
