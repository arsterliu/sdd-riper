...
const autonomyContract = require('../src/core/autonomy-contract');
...
function getMode(projectDir) {
  return governanceContract.defaults.mode;
}

function readProjectAutonomy(projectDir) {
  return autonomyContract.projectState({
    AUTONOMY_MODE: readConfigValue(projectDir, 'AUTONOMY_MODE'),
    APPROVAL_POLICY: readConfigValue(projectDir, 'APPROVAL_POLICY'),
    CRUISE_ENABLED: readConfigValue(projectDir, 'CRUISE_ENABLED')
  });
}
...
function getCruiseMaxIterations(projectDir) {
  ...
  return value > 0 ? value : governanceContract.defaults.cruiseMaxIterations;
}
...
// Auxiliary artifacts (design notes, execute logs, learning records) share the
// spec naming prefix but are not Specs; listing them as specs yields ghost
// entries and skews latest-spec selection.
function isAuxiliarySpecName(fileName) {
  return /^v\d+\.\d+(\.\d+)?-.+\.(design|execute|learning)\.md$/.test(String(fileName || ''));
}
...
function findLatestSpec(specsDir) {
  ...
  var files = fs.readdirSync(specsDir).filter(function(f) { return f.endsWith('.md') && f !== '.gitkeep' && !isAuxiliarySpecName(f); });
  var candidates = [];
...
function isFormalExecuteStep(line) {
  return /^Step(?: [^:]+)?:.*$/.test(line);
}

function isAllowedExecuteStepLine(line, activeStep) {
  return /^(?:Status|Result|Command|Files|Verification|Deviation|Timestamp|Cruise Iteration|Four-Axis Checklist|AC Coverage):(?:\s|$)/.test(line) ||
    !!(activeStep && activeStep.isCompletion && /^AC Coverage Summary:(?:\s|$)/.test(line));
}

function scanExecuteLog(executeLogContent) {
  var steps = [];
  var active = null;
  ...
  String(executeLogContent || '').split(/\r?\n/).forEach(function(line) {
    ...
    if (commentStart !== -1) {
      if (line.indexOf('-->', commentStart + 4) === -1) inHtmlComment = true;
      return;
    }
    if (/^\s*(?:```|~~~)/.test(line)) {
    ...
    if (isFormalExecuteStep(line)) {
      closeStep();
      active = {
        header: line,
        isCompletion: line === 'Step: completion-verification',
        lines: []
      };
      return;
    }
    ...
    if (/^---\s*$/.test(line) || /^#{1,6}(?:\s|$)/.test(line) ||
        (!line.startsWith(' ') && line.trim() && !isAllowedExecuteStepLine(line, active))) {
      closeStep();
      return;
    }
...
function lastFormalExecuteStep(executeLogContent) {
  var steps = scanExecuteLog(executeLogContent);
  return steps.length ? steps[steps.length - 1] : null;
}

function completionVerificationStatus(executeLogContent) {
  var lastStep = lastFormalExecuteStep(executeLogContent);
  if (!lastStep || !lastStep.isCompletion) return '';
  var status = '';
  lastStep.lines.forEach(function(line) {
    var match = line.match(/^Status:\s*([A-Z_]+)\s*$/);
...
module.exports = {
  ...
  getMode: getMode,
  readProjectAutonomy: readProjectAutonomy,
  ...
  isAuxiliarySpecName: isAuxiliarySpecName,
  ...
  scanExecuteLog: scanExecuteLog,
  lastFormalExecuteStep: lastFormalExecuteStep,
