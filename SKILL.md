...
This is the canonical rule text shared with the AI configs generated by `sdd init` (single source: `src/core/ai-config-rules.js`). Do not reword these lines independently — keep SKILL.md and the generated block consistent.
...
5. Apply the Spec intake rule above: reuse explicit, still-valid current-task inputs; ask only for missing or ambiguous information. Do not infer `version`, `task-name`, or autonomy mode silently. Bind supplied materials through `context-source` / `mydocs/context/<task-name>/`; an explicit statement that no other materials exist is sufficient.
...
- **Visual**: establish `ui-impact` for every Spec before routing visual intent. A baseline is the human-approved target UI PNG frozen by the current Spec, not a cross-Spec historical baseline library: a new Spec may directly use the latest UI PNG, while an old-page screenshot is optional Context. Recommend `fidelity` only for a decodable PNG with explicit scenario, route, state, and viewport, comparable target/current screenshot dimensions whose pixel width and height respectively match exactly, and stable test data, fonts, and assets; otherwise recommend `direction`. The Agent explains why it recommends `direction` or `fidelity`, and candidate images and default images do not constitute human approval. Strict visual evidence becomes active only when the current user explicitly runs `sdd visual init ...`; the Agent must never create, generate, approve, replace, version, or manage a baseline, must not automatically start a browser, and must not implicitly initialize or approve the visual Provider.
...
Autonomy write commands target only the current active Spec and re-read all expected digests under `.sdd-autonomy.lock`. `supervised` authorization binds both the current Scope and exact Plan digest. In `auto`, task authorization records main, worker, research-reviewer, and challenge-reviewer after the current user confirms scope and risk. When an Agent-approved Plan has no activation but those scope, risk, and Plan digests still match, the main Agent automatically runs `sdd autonomy activate-plan` with auditable `agent:<id>` evidence; it does not ask the user to approve the Plan again or re-authorize reviewers. A Plan rebind is valid only while Scope and risk remain unchanged. Never continue a native loop when `STOP_REASON` is non-empty.
...
Use `sdd codemap <dir>` for an on-demand architecture view, or archive only when relevant. Place external materials (PRD, UI mockups, API specs, SDK docs) in `mydocs/context/<task-name>/`; `sdd discover` auto-binds the matching directory as `context-source`. For ordinary evidence collection, weigh task boundaries, context cost, and independent evidence value using `protocols/subagent-dispatch.md`; read volume alone does not require delegation. When delegated, the subagent returns evidence and the orchestrator writes final Research content. Independent Research review remains required for standard/lite.
...
A baseline is the human-approved target UI PNG frozen by the current Spec, not a cross-Spec historical baseline library. A new Spec may directly use the latest UI PNG; an old-page screenshot is optional Context rather than a required historical baseline. Candidate images and default images do not constitute human approval, and the Agent must never create, generate, approve, replace, version, or manage a baseline.

The Agent recommends `fidelity` only when all prerequisites hold: the target is a decodable PNG; scenario, route, state, and viewport are explicit; target and post-development current screenshot dimensions are comparable, with pixel width and height respectively matching exactly; and test data, fonts, and assets are stable. Otherwise the Agent recommends `direction`, and it must explain why it recommends `direction` or `fidelity`.

`not-required` never blocks Plan because Context is empty. `direction` and `fidelity` become strict only after the current user explicitly runs `sdd visual init <dir> --spec <spec> --mode fidelity|direction`; then use `sdd visual inspect` before Plan approval. For an approved `fidelity` contract only, a project may configure the separate `playwright-visual` Provider with a static, project-local `sdd.visual.config.json` scenario mapping, then run `sdd verify visual <dir> --spec <spec>`. Each `scenario.baseline.path` must point into the current Spec Context and pass both lexical and realpath project-local containment. A configured mask is only a static pixel rectangle `{x,y,width,height}`; selectors and runtime expressions are not allowed. The command accepts no URL, command, selector, threshold, mask, or environment pass-through; it reads human-approved baselines, persists current/diff evidence in `mydocs/runs/visual/`, and never creates, approves, or replaces a baseline. It never starts a browser automatically. Provider, workspace, config, contract, baseline and code-state changes make prior evidence stale; a worktree mutation yields an auditable BLOCKED Run without current/diff attachments. Never fabricate a source, screenshot baseline, approval, browser result, or screenshot diff PASS. This contract does not create an Archive Gate.
...
- Design writing: **DELEGATABLE** (weigh shared context against the benefit of independently bounded module work)
...
No standalone Design. The five required Plan validation fields use these English labels:
...
Also record the actual `Selected Option` in one line for the existing archive summary. Add Scope, Touched Files, Change, or Blast Radius only when useful; do not repeat information already covered. Plan approval, execution evidence, and conditional E2E Provider requirements still apply.
...
```text
sdd challenge "<PROJECT_ROOT>" [--spec <path> | --name <slug|versioned-slug>]
```

`--spec` 与 `--name` 互斥。selector 必须解析到当前项目 docs root 的 `specs/` 下、文件名符合 Spec 版本命名且未归档的活动 Spec；docs root / `specs/` 目录本身及目标均须通过词法路径与 `realpath` 项目内包含检查，缺失、目录、非 Spec 文件、项目外或符号链接逃逸的目标必须失败且零写入。版本化 `--name` 精确定位，裸 `--name` 选择同 slug 的最新活动版本；没有 selector 时保持最新活动 Spec 的默认选择。生成 prompt 返回的结果写入命令固定携带项目相对 `--spec`，必须复用它，确保审查结果回写同一份 Spec。
...
3. 复用生成 prompt 中携带的精确目标，运行 `sdd challenge <project-dir> --spec <project-relative-spec-path> --record-result "VERDICT" --summary "summary" --executed-by "subagent:<id>"`
...
Run Learning Check after Challenge and before Archive. Ordinary `BUGFIX` and `DEVIATED_MINOR` alone do not require a Learning Record; keep their facts in Execute Log and create Learning when they yield a reusable rule. They never cancel another required trigger or a `FAIL_LEARNING` verdict.
...
- `sdd status <dir>`
- `sdd next <dir>`
- `sdd challenge <dir> [--spec <path>|--name <slug|versioned-slug>]`
